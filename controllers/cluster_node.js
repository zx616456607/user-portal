...
// host cpu
exports.getClustersCpu = function* () {
  const loginUser = this.session.loginUser
  const cluster = this.params.cluster
  const node = this.params.node
  const query = this.query
...
// host memory
exports.getClustersMemory = function* () {
  const loginUser = this.session.loginUser
  const cluster = this.params.cluster
  const node = this.params.node
  const query = this.query
  ...
  const result = yield api.getBy([cluster, 'metric', 'nodes', node, 'metrics'], memoryq)
  this.body = {
    memory: result[node]
  }
}

// host network/rx_rate and network/tx_rate
exports.getClustersNetwork = function* () {
  const loginUser = this.session.loginUser
  const cluster = this.params.cluster
  const node = this.params.node
  const query = this.query
  ...
  let resultArr = []
  resultArr.push(api.getBy([cluster, 'metric', 'nodes', node, 'metrics'], re_rateq))
  resultArr.push(api.getBy([cluster, 'metric', 'nodes', node, 'metrics'], te_rateq))
  const results = yield resultArr
  this.body = {
    rxRate: results[0][node],
    txRate: results[1][node]
  }
}

// host te_disk_read and te_disk_write

exports.getClustersDisk = function* () {
  const loginUser = this.session.loginUser
  const cluster = this.params.cluster
  const node = this.params.node
  const query = this.query
  ...
  let resultArr = []
  resultArr.push(api.getBy([cluster, 'metric', 'nodes', node, 'metrics'], te_disk_read))
  resultArr.push(api.getBy([cluster, 'metric', 'nodes', node, 'metrics'], te_disk_write))
  const results = resultArr
  this.body = {
    diskReadIo: results[0][node],
    diskWriteIo: results[1][node]
  }
}
