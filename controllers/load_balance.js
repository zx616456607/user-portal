...
exports.editLBConfig = function* () {
  ...
  const loginUser = this.session.loginUser
  const api = apiFactory.getK8sApi(loginUser)
  const result = yield api.updateBy([cluster, 'loadbalances', name, 'displayname', displayName, 'agentType', agentType, 'config'], null, body)
  this.body = result
}

exports.getLBConfig = function* () {
  const cluster = this.params.cluster
  const name = this.params.name
  const displayName = this.params.displayname
  const agentType = this.params.agentType
  const loginUser = this.session.loginUser
  const api = apiFactory.getK8sApi(loginUser)
  const result = yield api.getBy([cluster, 'loadbalances', name, 'displayname', displayName, 'agentType', agentType, 'config'], null)
  this.body = result
}
