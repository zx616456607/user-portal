...
import { Modal, Input, Row, Form, Spin, Icon } from 'antd'
import '../ClusterModule/ClusterAutoScale/style/tabModal.less'
...
export default connect(mapStateToProps, {
  ...
  componentDidMount() {
    const { currData, isEdit } = this.props
    if (isEdit && !!currData) {
      this.setState({
        disabled: true,
        currentIcon: currData.iaasType,
      })
    }
    this.getServers()
    this.setCurrDis()
  ...
  onTab2ModalOk = () => {
    ...
    form.validateFields((errors, values) => {
      ...
      this.setState({
        ...
        // 新增、修改接口
        const { addServer, updateServer, funcTab1, scope, currData } = this.props
  ...
  render() {
    ...
    const { currentIcon, currDis, submitLoading } = this.state
    ...
    const userName = currData ? currData.userName : ''
          ...
          <Form horizontal >
            ...
            </div>
            {
              currentIcon === 'openstack' || currentIcon === 'vmware' ?
                [
                  <div key="line" className="bottom-line"></div>,
                  <div key="formDiv" className="formContainer" style={{ paddingTop: 20 }}>
                    <Row key="name">
                      <FormItem
                        {...formItemLargeLayout}
                        label="资源池名称"
                      >
                        <Input disabled={isEdit} {...getFieldProps('name', { initialValue: name,
                          validate: [{
                            rules: [
                              { required: true, message: '请输入资源池名称' },
                              { validator: this.onCheckServerName },
                            ],
                    ...
                    <Row key="row3">
                      <FormItem
                        ...
                        label="登录用户名"
                      >
                        <Input {...getFieldProps('Username', { initialValue: userName,
                          validate: [{
                            rules: [
