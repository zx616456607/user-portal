...
import { createConfigFiles, deleteConfigGroup, deleteConfigFiles, addConfigFile, fetchDelConfigMap } from '../../actions/configs'
...
class CollapseHeader extends Component {
  ...
  btnDeleteGroup() {
    ...
    let configData = {
      ...
      // "groups": configArray
      configmap_name: this.state.groupName,
      cluster_id: this.props.cluster.clusterID,
    }
    let notification = new NotificationHandler()
    // self.props.deleteConfigGroup(configData, {
    self.props.deleteConfigMap(this.props.cluster.clusterID, {
      list: [this.state.groupName]
    }, {
      success: {
        func: (res) => {
          // const errorText =[]
  ...
  render() {
    ...
    const realConfigNameList = currConfigMap ? currConfigMap.configList : []
    return (
      <Row>
...
function mapStateToProps(state, props) {
  ...
  const {configFiles, isFetching, configNameList} = configGroupList[cluster.clusterID] ? {
    configFiles: configGroupList[cluster.clusterID].configGroup || [],
    configNameList: configGroupList[cluster.clusterID].configGroup.map(item => item.name) || [],
    isFetching: configGroupList[cluster.clusterID].isFetching
  } : defaultConfigFiles
  return {
    configFiles,
    cluster,
...
function mapDispatchToProps(dispatch) {
  return {
    ...
    deleteConfigMap: (cluster_id, body, callback) => { dispatch(fetchDelConfigMap(cluster_id, body, callback)) },
