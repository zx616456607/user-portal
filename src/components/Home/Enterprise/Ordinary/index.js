          ...
          <Col span={6} className='quota'>
            <Card title="项目&集群相关资源配额" bordered={false} bodyStyle={{ height: 220, padding: '1px' }}
              extra={<Link to={spaceName === '我的个人项目' ? this.props.loginUser.role !== 2 ?
                '/account?tabs=3' : `/tenant_manage/user/${this.props.loginUser.userID}?tabs=3` :
                this.props.userID === undefined ? `/tenant_manage/project_manage/project_detail?name=${this.props.projectName}&tabs=3` :
                  `/tenant_manage/user/${this.props.userID}?tabs=3`}><Button type="primary" size="small">{this.props.loginUser.role === 2 ? '设置配额' : '查看详情'}</Button></Link>}>
