    ...
    if(Object.keys(_POST).length == 0) {//判断POST来的数组是否为空
        ...
        if (null != _POST["notify_id"]) {
            this.getResponse(_POST["notify_id"], function(responseTxt){
                core_funcs.logResult('getResponse(_POST["notify_id"]:' + responseTxt);
...
AlipayNotify.prototype.verifyReturn = function(_GET, callback){
    if(Object.keys(_GET).length == 0) {//判断POST来的数组是否为空
        core_funcs.logResult('Object.keys(_GET).length == 0');
        callback(false);
    }
    else{
        //生成签名结果
        var isSign = this.getSignVeryfy(_GET, _GET["sign"]);
        core_funcs.logResult('var isSign = this.getSignVeryfy(_GET, _GET["sign"]):' + isSign);
        //获取支付宝远程服务器ATN结果（验证是否是支付宝发来的消息）
        var responseTxt = 'true';
        //验证
        //responsetTxt的结果不是true，与服务器设置问题、合作身份者ID、notify_id一分钟失效有关
        ...
        if (null != _GET["notify_id"]) {
            this.getResponse(_GET["notify_id"], function(responseTxt){
                core_funcs.logResult('getResponse(_GET["notify_id"]:' + responseTxt);
                callback(responseTxt == 'true' && isSign);
            });
        }
        else{
...
AlipayNotify.prototype.getSignVeryfy = function(para_temp, sign){
    ...
    // var para_sort = core_funcs.argSort(para_temp);

    //把数组所有元素，按照“参数=参数值”的模式用“&”字符拼接成字符串
    var prestr = core_funcs.createLinkstring(para_sort);
    var isSgin = false;

    var sign_type_config = this.alipay_config['sign_type'].trim().toUpperCase();
    console.log('this.alipay_config[sign_type].trim().toUpperCase():' + sign_type_config);
    // var sign_type_config = 'MD5';
    if(sign_type_config === 'MD5'){
